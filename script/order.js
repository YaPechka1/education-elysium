...
async function getItems() {
        ...
        await fetch('/func/getOrderOne.php?id='+myParam).then(res => res.json()).then(
            async (data) => {
                items = data;
                for (let i = 0; i < data.length; i++) {
                    it_container.insertAdjacentHTML('beforeend', '<div class="container"><h5 class="block">' + data[i].item_name + '</h5><img class="block" src="' + data[i].logo + '"><select class="block" disabled data-val="' + data[i].status + '">'+option+'</select><input type="text" class="block" readonly  value="Количество: ' + data[i].count + '"><input type="text" class="block" readonly value="Цена: ' + data[i].price + ' руб."><input type="text" class="block" readonly value="Категория: ' + data[i].cat_name + '"><input type="text" class="block" readonly value="Магазин: ' + data[i].shop_name + '"><a href="/item.php?id=' + data[i].id + '"><a href="/item.php?id=' + data[i].id_item + '"><button class="btn">Подробнее</button></a></div>');
                }
                selectEdit();
            }
        )
